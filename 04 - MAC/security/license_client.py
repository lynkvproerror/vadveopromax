...
class HardwareFingerprint:
    ...
    def get_all_components() -> Dict[str, str]:
        """Collect all hardware identifiers (macOS version)"""
        components = {}
        ...
        try:
            ...
            for line in output.split('\n'):
                if 'IOPlatformSerialNumber' in line:
                    components['mb_serial'] = line.split('=')[-1].strip().strip('"')
                    break
            if 'mb_serial' not in components:
                components['mb_serial'] = 'unknown'
        except:
            components['mb_serial'] = 'unknown'
        
        # 3. Hardware UUID (equivalent to motherboard UUID)
        try:
            output = subprocess.check_output(
                ['ioreg', '-rd1', '-c', 'IOPlatformExpertDevice'], stderr=subprocess.DEVNULL
            ).decode()
            for line in output.split('\n'):
                if 'IOPlatformUUID' in line:
                    components['mb_uuid'] = line.split('=')[-1].strip().strip('"')
                    break
            if 'mb_uuid' not in components:
                components['mb_uuid'] = 'unknown'
        except:
            components['mb_uuid'] = 'unknown'
        
        # 4. BIOS Serial (on Mac, same as platform serial)
        components['bios_serial'] = components.get('mb_serial', 'unknown')
        
        # 5. Boot Disk identifier
        try:
            output = subprocess.check_output(
                ['diskutil', 'info', '/'], stderr=subprocess.DEVNULL
            ).decode()
            for line in output.split('\n'):
                if 'Volume UUID' in line or 'Disk / Partition UUID' in line:
                    components['disk_serial'] = line.split(':')[-1].strip()
                    break
            if 'disk_serial' not in components:
