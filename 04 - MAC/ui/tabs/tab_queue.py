...
class TabQueue(
    ...
    def _on_progress_update(self, task_id: str, progress: int, status_text: str = ""):
        ...
        # ★ Fix O1: Per-task progress throttle (150ms gate)
        # Skip rapid-fire updates for same task to prevent 40+ setStyleSheet
        # calls during burst completions. Always process completions & transitions.
        is_completion = progress >= 100
        is_transition = status_text and any(
            c in status_text for c in ("✨", "🔧", "⬆️", "🔄", "📥")
        )
        if not is_completion and not is_transition:
    ...
    def _execute_post_queue_action(self, action: str):
        ...
        if action == "shutdown":
            ...
            # macOS: schedule shutdown in 1 minute
            try:
                subprocess.Popen(
                    ["osascript", "-e",
                     'do shell script "shutdown -h +1" with administrator privileges'],
                )
            except Exception as e:
            ...
            if main_window and hasattr(main_window, 'show_toast'):
                main_window.show_toast(
                    ...
                    "Click Cancel below or run 'sudo killall shutdown' to abort.",
    ...
    def _do_sleep(self):
        """Execute sleep command (macOS)."""
        import subprocess
        import logging
        try:
            # macOS: pmset sleepnow puts the Mac to sleep immediately
            subprocess.Popen(["pmset", "sleepnow"])
        except Exception as e:
            logging.getLogger("queue").error(f"[Queue] Sleep command failed: {e}")
    
    def _cancel_post_queue_action(self):
        """Cancel a pending shutdown (macOS)."""
        import subprocess
        import logging
        try:
            # macOS: kill the pending shutdown process
            subprocess.Popen(["sudo", "killall", "shutdown"])
            self._post_queue_triggered = False
