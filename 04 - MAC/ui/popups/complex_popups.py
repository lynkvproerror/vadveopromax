...
class AddProfileDialog(BasePopup):
    ...
    def _create_content(self):
        ...
        if self._detected_profiles:
            ...
            self.path_entry.setPlaceholderText("~/Library/Application Support/Google/Chrome/Default")
...
class _DraggableImageCard(QFrame):
    ...
    def mouseMoveEvent(self, event):
        ...
        # macOS: no ctypes needed for window management
        popup = self.window()
        ...
        if self._had_stay_on_top:
            pass  # macOS: SetWindowPos not available
# Grab mouse globally — all mouse events come to this widget
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton and self._is_manual_dragging:
            ...
            # Restore popup topmost status (macOS: no-op, Qt handles this)
            if self._had_stay_on_top:
                pass  # macOS: SetWindowPos not available — Qt window flags suffice
