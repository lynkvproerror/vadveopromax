"""
VEO Pro Max — Persistent Chrome Manager (macOS)

Manages Chrome processes that survive app restarts.
Launch via subprocess.Popen (start_new_session=True), reconnect via CDP port.
...
# macOS: detect arm64 (Apple Silicon) vs x64 (Intel)
CFT_PLATFORM = "mac-arm64" if platform.machine() == "arm64" else "mac-x64"
...
def _find_cft_exe() -> Optional[str]:
    ...
    # CfT zip extracts to chrome-mac-arm64/ or chrome-mac-x64/ on macOS
    for variant in ["chrome-mac-arm64", "chrome-mac-x64"]:
        candidate = (
            cft_dir / variant
    ...
    flat = cft_dir / "Google Chrome for Testing"
    if flat.exists():
        return str(flat)
    return None
...
def _find_branded_chrome() -> Optional[str]:
    ...
    for p in common_paths:
        if p.exists():
            # Exclude CfT binary that may be at a similar path
            if "chrome-for-testing" not in str(p).lower():
                log.info(f"[ChromeManager] Found branded Chrome at: {p}")
                return str(p)
    
    # Also try `which` for command-line installed Chrome
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            log.info(f"[ChromeManager] Found Chrome via which: {path}")
            return path
    except Exception:
        pass
...
def _find_user_chrome_local_state() -> Optional[Path]:
    """Find user's main Chrome Local State file (macOS)."""
    candidates = [
