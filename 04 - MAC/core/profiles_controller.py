...
# ── macOS helpers for browser window management ────────────────────
import subprocess as _sp


def _find_hwnds_by_pid(pid: int) -> list:
    """Find Chrome windows by PID (macOS stub).
    
    macOS doesn't expose HWND handles. Returns list of PIDs
    representing Chrome processes in the process tree.
    """
    if not pid:
        return []
    pids = [pid]
...
def _find_chrome_pid_by_profile(profile_dir_name: str) -> int:
    ...
    try:
        result = _sp.run(
            ['pgrep', '-f', f'Google Chrome.*{profile_dir_name}'],
        ...
        log.error(f"[macOS] PID lookup error: {e}")
    return 0

def _win32_hide_hwnds(hwnds: list):
    """Hide Chrome windows on macOS via AppleScript."""
    try:
        _sp.run(
            ['osascript', '-e',
...
def _win32_show_hwnds(hwnds: list):
    """Show and activate Chrome windows on macOS."""
    try:
        _sp.run(
            ['osascript', '-e',
...
def _win32_send_to_back(hwnds: list):
    """Send Chrome windows behind all other windows on macOS."""
    try:
        _sp.run(
            ['osascript', '-e',
             'tell application "System Events" to set visible of '
...
class ProfilesController:
    ...
    def auto_login_with_credentials(
        ...
        try:
            ...
            try:
                ...
                # Kill Chrome processes that reference this profile path (macOS)
                import subprocess
                try:
                    subprocess.run(
                        ["pkill", "-f", f"Google Chrome.*{profile_path.name}"],
    ...
    def copy_variations_and_warmup(self, email: str) -> bool:
        ...
        try:
            ...
            # Kill Chrome processes using this profile (macOS)
            try:
                subprocess.run(
                    ["pkill", "-f", f"Google Chrome.*{profile_path.name}"],
                    capture_output=True, timeout=10,
