...
def _get_default_chrome_user_data_dir() -> Optional[str]:
    """Get user's default Chrome User Data directory on macOS."""
...
def _extract_via_temp_chrome() -> Optional[str]:
    ...
    try:
        ...
        # Fully hide Chrome window via AppleScript
...
def _hide_process_windows(pid: int, retries: int = 5):
    """Hide Chrome windows on macOS (uses AppleScript).
    
    Hides all Chrome windows from the Dock and screen.
    """
    try:
        ...
        log.debug(f"[ClientDataExtractor] Window hide failed: {e}")
