"""
VEO Pro Max - Dependency Checker (macOS)

Checks and installs missing Python packages + Google Chrome
during startup. All subprocess calls use standard POSIX conventions.
"""
...
def _hidden_startupinfo():
    """Return None on macOS (STARTUPINFO is Windows-only)."""
    return None


def _hidden_flags() -> int:
    """Return 0 on macOS (CREATE_NO_WINDOW doesn't exist)."""
    return 0
...
def _install_pip_package(pip_name: str) -> bool:
    ...
    NOTE: Skipped in compiled (py2app/frozen) mode — all packages
...
def _find_chrome_path() -> Optional[str]:
    """Detect Google Chrome installation on macOS.
    
    Checks:
    1. Standard application paths
    2. Homebrew cask installation
    
    Returns path to Chrome binary if found, None otherwise.
    """
    # Method 1: Standard macOS application paths
    common_paths = [
    ...
    # Method 2: Check via `which`
    try:
        result = subprocess.run(
            ["which", "google-chrome"], capture_output=True, text=True, timeout=3
        )
...
def _install_chrome(callback: Optional[Callable] = None) -> bool:
    """Prompt user to install Google Chrome on macOS.
    
    Unlike Windows, macOS doesn't support silent MSI install.
    Opens the Chrome download page for the user.
    Returns False (manual action required).
    """
    try:
        if callback:
            callback(55, "⚠️ Google Chrome not found")
        
        log.warning("[DependencyChecker] Chrome not found — prompting user to install")
        ...
        if callback:
            callback(65, "⚠️ Please install Google Chrome from google.com/chrome")
        
        log.warning("  ⚠️ Please install Google Chrome manually from https://www.google.com/chrome/")
        return False
            
    except Exception as e:
        log.error(f"  ✗ Chrome install prompt error: {e}")
        return False
...
def check_all_dependencies(callback: Optional[Callable] = None):
    ...
    if chrome_path:
        ...
        log.warning("  Chrome NOT found — prompting user...")
        success = _install_chrome(callback)
        if success:
            if callback:
                callback(100, "✅ Google Chrome installed")
        else:
            log.error("  Chrome not available — browser features may not work")
            if callback:
                callback(100, "⚠️ Chrome not found — install from google.com/chrome")
