...
class AntiDebug:
    ...
    def is_debugger_present() -> bool:
        """Check if debugger is attached (macOS)."""
        try:
            import ctypes
            import ctypes.util
            import struct
...
class EnvironmentChecker:
    ...
    def check_vm_registry() -> bool:
        """Check for VM indicators (macOS version)."""
        try:
            import subprocess
            # Check if running inside a VM via ioreg
            result = subprocess.run(
                ['ioreg', '-l'],
                capture_output=True, text=True, timeout=5,
            )
            output = result.stdout.upper()
            vm_indicators = ["VMWARE", "VIRTUALBOX", "PARALLELS", "QEMU"]
    ...
    def check_system_manufacturer() -> bool:
        """Check system manufacturer for VM names (macOS)."""
        try:
            import subprocess
            result = subprocess.run(
