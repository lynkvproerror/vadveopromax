"""
VEO Pro Max — Extension Manager (CDP-based, macOS)

Manages Chrome extension install/uninstall/reinstall using CDP WebSocket
and AppleScript for OS dialog automation.

Strategy:
- Install: CDP Runtime.evaluate on chrome://extensions shadow DOM
           + AppleScript for macOS folder picker dialog (Cmd+Shift+G)
...
def _get_safe_path(folder_path: str) -> str:
    """Copy extension to a clean TEMP path for folder picker dialogs.
    ...
    $TMPDIR/veo_extension (clean path, no special chars).
...
# ── macOS Folder Picker (AppleScript) ────────────────────────────────────

def _type_folder_path_macos(folder_path: str, max_retries: int = 2) -> bool:
    """Automate macOS NSOpenPanel folder picker via AppleScript.
    
    When Chrome opens the "Load Unpacked" folder picker (NSOpenPanel),
    this function uses AppleScript to:
    1. Wait for the Open dialog to appear
    2. Press Cmd+Shift+G to open "Go to Folder" sheet
    3. Clear any existing text and type the extension path
    4. Press Return to navigate to the folder
    5. Press Return again to confirm selection (click "Open")
    
    Args:
        folder_path: Absolute path to extension directory
        max_retries: Number of retry attempts
    
    Returns:
        True if folder was selected successfully, False on failure.
    """
    import subprocess
    
    # Use safe path (copy to temp if special chars in original path)
    safe_path = _get_safe_path(folder_path)
    
    for attempt in range(max_retries + 1):
        try:
            attempt_label = f"attempt {attempt + 1}/{max_retries + 1}"
            log.info(f"[ExtMgr] 📂 macOS folder picker ({attempt_label}): {safe_path}")
            
            # AppleScript to automate the folder picker dialog
            # Strategy: Cmd+Shift+G → "Go to Folder" → type path → Enter × 2
            applescript = f'''
                tell application "System Events"
                    -- Wait for the Open dialog to appear (up to 5s)
                    set dialogFound to false
                    repeat 10 times
                        try
                            tell process "Google Chrome"
                                set frontWin to front window
                                -- Check if there's a sheet (the Open dialog is a sheet)
                                if (count of sheets of frontWin) > 0 then
                                    set dialogFound to true
                                    exit repeat
                                end if
                                -- Also check for a standalone dialog window
                                set winCount to count of windows
                                if winCount > 0 then
                                    set winName to name of front window
                                    if winName contains "Open" or winName contains "打开" or winName contains "Mở" then
                                        set dialogFound to true
                                        exit repeat
                                    end if
                                end if
                    ...
                    if dialogFound is false then
                        error "Open dialog not found after 5 seconds"
                    end if
                    ...
                    tell process "Google Chrome"
                        ...
                        -- Step 1: Cmd+Shift+G to open "Go to Folder" sheet
                        keystroke "g" using {{command down, shift down}}
                        delay 0.8
                        
                        -- Step 2: Clear existing text and type the path
                        keystroke "a" using {{command down}}
                        delay 0.1
                        keystroke "{safe_path}"
                        delay 0.3
                        
                        -- Step 3: Press Return to navigate to the folder
                        key code 36
                        delay 1.0
                        
                        -- Step 4: Press Return again to confirm (click "Open")
                        key code 36
                        delay 0.5
                    end tell
                end tell
            ...
            if result.returncode == 0 and 'ok' in result.stdout:
                log.info(f"[ExtMgr] ✅ Folder picker completed successfully")
                return True
            else:
                stderr = result.stderr.strip() if result.stderr else 'unknown'
                log.warning(f"[ExtMgr] ⚠️ Folder picker failed ({attempt_label}): {stderr}")
                
                if attempt < max_retries:
                    log.info(f"[ExtMgr] Retrying in 2s...")
                    time.sleep(2)
                    continue
                    
        except subprocess.TimeoutExpired:
            log.error(f"[ExtMgr] ❌ Folder picker timed out ({attempt_label})")
        except Exception as e:
            log.error(f"[ExtMgr] ❌ Folder picker error ({attempt_label}): {e}")
    
    log.error("[ExtMgr] ❌ All folder picker attempts failed")
    return False


# ── Public API ──────────────────────────────────────────────────────────

def install_extension(port: int, extension_dir: str, **kwargs) -> bool:  # noqa: C901
    """Install unpacked extension into Chrome via CDP + AppleScript.
    
    Thread-safe: uses per-port lock to prevent concurrent installs.
    Handles hidden browsers: shows Chrome temporarily for dialog.
    
    Steps:
    1. Open chrome://extensions via CDP
    2. Toggle Developer Mode ON via shadow DOM JS
    3. Click "Load unpacked" via shadow DOM JS
    4. Handle folder picker dialog via AppleScript (Cmd+Shift+G)
...
def _show_chrome_for_install(port: int) -> bool:
    """Show Chrome window if hidden (macOS: use AppleScript).
...
def _hide_chrome_after_install(port: int):
    """Re-hide Chrome window after install (macOS: use AppleScript)."""
    try:
        import subprocess
        subprocess.run(
            ['osascript', '-e',
             'tell application "System Events" to set visible of '
             'every process whose name is "Google Chrome" to false'],
            capture_output=True, timeout=5,
        )
    except Exception:
        pass
...
def _install_extension_impl(port: int, extension_dir: str) -> bool:
    ...
    try:
        ...
        # Step 4: Handle folder picker via AppleScript (macOS)
        picker_ok = _type_folder_path_macos(extension_dir)
