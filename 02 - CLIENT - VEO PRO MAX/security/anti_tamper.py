...
class AntiExtraction:
    ...
    def check() -> dict:
        ...
        # Check 1: Running under python.exe (not compiled binary)
        exe_name = os.path.basename(sys.executable).lower()
        if exe_name in ("python.exe", "python3.exe", "pythonw.exe"):
        ...
        temp_indicators = ["\\Temp\\", "\\tmp\\", "/tmp/", "\\AppData\\Local\\Temp\\"]
...
class ProcessVerifier:
    ...
    # Trusted parent processes on Windows
    TRUSTED_PARENTS = {
        "explorer.exe", "cmd.exe", "powershell.exe",
        "windowsterminal.exe", "conhost.exe",
        "code.exe", "cursor.exe",  # IDEs (dev mode)
        "windsurf.exe", "trae.exe",
        "python.exe", "pythonw.exe",  # Dev mode
    }
    ...
    def check() -> dict:
        ...
        try:
            ...
            # Alternative: use os.getppid() (Python 3.2+)
            ppid = os.getppid()
...
class ProxyDetector:
    ...
    def check() -> dict:
        ...
        # Check 1: Windows Registry proxy settings
        try:
            import winreg
            key = winreg.OpenKey(
            ...
            if proxy_enable:
                ...
                proxy_info = str(proxy_server)
                # Common MITM proxy ports
                mitm_ports = ["8080", "8888", "8443", "9090", "8082"]
                if any(port in proxy_info for port in mitm_ports):
...
class SandboxDetector:
    """Detect Sandboxie and Windows Sandbox environments."""
    ...
    def check() -> dict:
        ...
        try:
            ...
            # Check 1: Sandboxie DLL loaded
            sbie_dll = ctypes.windll.kernel32.GetModuleHandleW("SbieDll.dll")
            if sbie_dll:
                flags.append("sandboxie_dll")
        except Exception:
            pass
