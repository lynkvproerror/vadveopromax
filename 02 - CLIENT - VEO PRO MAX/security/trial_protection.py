...
class TrialMarkerManager:
    ...
    def _write_firebase_marker(self, trial_start: datetime):
        ...
        Uses register_trial() from FirebaseRESTClient to create server-side record.
        """
        try:
            try:
            ...
            client = FirebaseRESTClient()
            client.register_trial(self.machine_id)
        except Exception:
            pass  # Non-fatal: local markers still work
    
    def _read_firebase_marker(self) -> Optional[datetime]:
        """Read trial start from Firebase via REST API.
        Uses check_trial_status() — also detects admin revocation.
        """
        try:
            try:
                from security.firebase_rest_client import FirebaseRESTClient
            ...
            client = FirebaseRESTClient()
            status = client.check_trial_status(self.machine_id)
            
            if not status.get("exists"):
                return None
            
            # Admin revoked → return None (treated as "never trialed")
            if status.get("status") in ("revoked", "expired"):
                return None
            
            started = status.get("started_at") or status.get("expires_at")
            if started:
                try:
            ...
            return None  # Network error — fallback to local markers
...
def activate_trial_key(machine_id: str, trial_key: str) -> TrialStatus:
    """
    Activate a trial key (local marker setup).
    
    Trial key format: TRIAL-XXXX-XXXX (generated by admin)
    NOTE: Server-side validation is done in license_client.activate_trial()
    before calling this function. This only handles local marker creation.
    """
