...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action === 'get_recaptcha') {
        handleGetRecaptcha(msg, sendResponse);
...
async function handleGetRecaptcha(msg, sendResponse) {
    try {
        // Check if reCAPTCHA Enterprise is available
        const hasRecaptcha = eval(RECAPTCHA_CHECK);
        if (!hasRecaptcha) {
            sendResponse({ token: null, error: 'reCAPTCHA Enterprise not available on page' });
            return;
        }

        // Get site key
        const siteKey = msg.siteKey || extractSiteKey();
        if (!siteKey) {
            sendResponse({ token: null, error: 'Could not extract reCAPTCHA site key' });
            return;
        }

        console.log(`[VEO Bridge Content] Executing reCAPTCHA Enterprise: key=${siteKey.substring(0, 16)}...`);

        // Execute on the REAL page context
        const token = await grecaptcha.enterprise.execute(siteKey, { action: 'VIDEO_GENERATION' });

        if (token && token.length > 500) {
            console.log(`[VEO Bridge Content] ✅ reCAPTCHA token: ${token.length} chars`);
            sendResponse({ token });
        } else {
            console.warn(`[VEO Bridge Content] ⚠️ Token too short: ${token ? token.length : 0} chars`);
            sendResponse({ token: null, error: `Token too short (${token ? token.length : 0} chars)` });
        }
    } catch (e) {
        console.error('[VEO Bridge Content] reCAPTCHA execute failed:', e);
